...
We perform causal interventions by hooking into the model's forward pass at layer 10 and modifying SAE-encoded features. For \emph{amplification}, we encode the residual activation through the SAE, multiply the top-20 Portuguese-specific features by factor $\alpha \in \{1, 2, 5, 10\}$, decode back, and replace the original activation; this tests whether increasing PT feature magnitudes forces Portuguese generation. For \emph{suppression}, the same procedure is applied but the top-20 features are set to zero, testing whether removing PT features causes language switching and, in particular, which language the model defaults to.
