...
The choice of Portuguese is methodological rather than incidental. Its Romance neighbors (Spanish, Catalan, French, Italian) make any Romance fallback directly visible: when suppression redirects Gemma to Spanish or Catalan, we are reading off the model's next-nearest language in representational space. A typologically isolated probe language with no close neighbors in the evaluation set would conflate the continuum with the substrate, since both would produce ``English or noise'' as the only accessible fallback. Portuguese also has over 260 million speakers worldwide, with Brazilian Portuguese the dominant variety and a language under-represented in mechanistic interpretability research; the choice therefore targets both methodological sensitivity and linguistic importance.

We make three contributions. First, we show that two qualitatively distinct organizations of multilinguality coexist in current LLMs---the Romance-language continuum and the English substrate---and we establish the contrast through causal feature-suppression rather than correlational analysis. Second, we argue that the direction of generation collapse under suppression is itself a readout of a language's representational neighborhood, and that pairing this \emph{lesion-based readout} with a typologically-embedded probe language is what makes the paradigms distinguishable. Third, three complementary measurements independently corroborate the divide: raw-neuron monolinguality (0.270 Llama against 0.252 Gemma, gap widening with depth), pairwise FLORES similarity showing typological distance from the earliest layers, and individual-feature steering that exposes a PT-EN bridge feature in Llama with no Gemma analogue.
...
The two paradigms suggest different affordances: a Romance continuum may permit finer-grained language steering by ``sliding'' between related languages, while an English substrate raises concerns about English-centric biases propagating to all other languages. More broadly, training data proportions appear to shape internal language architecture, not just downstream performance.
