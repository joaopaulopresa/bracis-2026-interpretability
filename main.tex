...
% ============================================================
\begin{abstract}
How do multilingual large language models organize the languages they speak? We present causal evidence that the answer depends on the model family. Using Portuguese as a probe, we apply Sparse Autoencoder (SAE) feature-suppression to Gemma-2-2B and Llama-3.1-8B: ablating the top Portuguese-specific features redirects Gemma to Spanish or Catalan, but redirects Llama to English. The two fallback directions expose two internal geometries that we name the \emph{Romance-language continuum} (languages arranged by typological proximity, as observed in Gemma) and the \emph{English substrate} (non-English languages as overlays on an English default, as observed in Llama). Portuguese is well-suited to surface this contrast: its Romance neighbors make any Romance fallback directly observable, which a typologically isolated probe language could not reveal. Three converging measurements support the divide. A monolinguality analysis of raw neurons yields 0.270 for Llama against 0.252 for Gemma, with a widening gap in deeper layers; pairwise FLORES similarity places typological distance at every depth but refines Romance-internal distinctions only in late layers; and amplifying one Llama-Scope feature (\#18397) produces PT-EN hybrid text, offering a feature-level instance of the English-pivot hypothesis. We also train 16 custom SAEs and observe that the ReLU+L1 vs TopK architecture ranking does not transfer across base models (TopK FVE 79.7\% on Gemma against 98.0\% on Llama). To our knowledge, this is the first mechanistic study of multilingual representation to contrast paradigms across model families rather than describe one in isolation.
...
This paper offers causal evidence that both pictures are correct---each for a different model family. Using Portuguese as a probe in Sparse Autoencoder (SAE) feature-suppression experiments (Figure~\ref{fig:two_architectures}), we find that suppressing the top Portuguese-specific features in Gemma-2-2B redirects generation to Spanish or Catalan, while the same intervention in Llama-3.1-8B redirects to English. The two fallback directions expose two internal geometries: the \emph{Romance-language continuum}, in which languages are organized by typological proximity (observed in Gemma), and the \emph{English substrate}, in which non-English languages behave as overlays on top of an English default (observed in Llama).
...
Portuguese is methodological rather than incidental. Its Romance neighbors (Spanish, Catalan, French, Italian) make any Romance fallback directly visible, so suppression redirecting Gemma to Spanish or Catalan reads off the model's next-nearest language in representational space; a typologically isolated probe with no close neighbors would conflate the continuum with the substrate. Portuguese has over 260 million speakers, with Brazilian Portuguese the dominant variety and an under-represented language in mechanistic interpretability work.

We make three contributions. First, we show that two qualitatively distinct organizations of multilinguality coexist in current LLMs---the Romance-language continuum and the English substrate---and we establish the contrast through causal feature-suppression rather than correlational analysis. Second, we argue that the direction of generation collapse under suppression is itself a readout of a language's representational neighborhood, and that pairing this \emph{lesion-based readout} with a typologically-embedded probe language is what makes the paradigms distinguishable. Third, three complementary measurements---raw-neuron monolinguality, pairwise FLORES similarity, and single-feature steering---independently corroborate the divide, including a PT-EN bridge feature in Llama with no Gemma analogue.

% ============================================================
\section{Related Work}
\label{sec:related}

Sparse autoencoders decompose activations into overcomplete, sparse features that are more interpretable than raw neurons~\cite{cunningham2023sparse,bricken2023monosemanticity}, with training recipes, the FVE/dead-feature metrics we adopt, and a recent survey in~\cite{gao2024scaling,deng2025survey}; benchmarks~\cite{karvonen2025saebench,wu2025axbench} caution that proxy metrics do not always predict downstream utility, though SAEs remain well-suited to feature-level analysis. We build on pretrained SAE suites for Gemma~\cite{lieberum2024gemma} and Llama~\cite{he2024llama} and extend feature-clamping~\cite{bricken2023monosemanticity} to language identity.

The multilingual-representation literature is split. One line argues that Llama-family models reason through English-centric intermediates: Wendler et al.~\cite{wendler2024llamas} use logit-lens analyses on Llama~2, and Schut et al.~\cite{schut2025english} extend the picture with activation steering, reporting that English-derived steering vectors transfer better than those from other languages. A separate line~\cite{kojima2024multilingual,tang2024language,deng2025unveiling} documents shared multilingual spaces where languages cluster by typological similarity, and the closest cross-family concurrent work---Brinkmann et al.~\cite{brinkmann2025shared}, training SAEs on Llama-3-8B and Aya-23-8B---shows that morphosyntactic concepts (number, gender, tense) live on feature directions shared across typologically diverse languages. We read these pictures as compatible once paired with a model family: grammatical abstraction may be shared cross-lingually while language-identity geometry remains family-dependent. Our contribution makes that divide causal and family-contrastive---suppression redirects Gemma to a Romance neighbor and Llama to English, with a typologically embedded probe (Portuguese) as what makes the two paradigms distinguishable. Concurrent SAE-steering studies target typologically distant pairs within a single model (EN$\to$ZH, EN$\to$JA): LangFIR~\cite{langfir2026}, SASFT~\cite{sasft2026}, and Causal Language Control~\cite{causal_lang_control2025} (90\% single-feature steering in Gemma-2B); our setting differs by being cross-family and Romance-focused, where shared representations make steering harder. On Portuguese itself, prior NLP work targets downstream performance~\cite{souza2020bertimbau}; we provide the first mechanistic catalog of Portuguese-specific features with causal validation.
...
Table~\ref{tab:steering} delivers the central result of the paper: suppressing the top-20 PT-specific features redirects generation in family-specific directions, and the direction is what separates the two paradigms of Figure~\ref{fig:two_architectures}.
...
A pairwise cosine similarity analysis on mean FLORES activations supports a layered typological separation in both models: PT-EN distance (Romance vs.\ Germanic) is largest from the earliest layers, PT-FR distance (cross-subfamily) grows in middle layers, and PT-ES distance (Iberian) diverges only in late layers. For Gemma~2B at layer~25 we measure cos(PT,ES)$=0.958$, cos(PT,FR)$=0.928$, cos(PT,EN)$=0.932$. The continuum therefore has variable granularity across depth.

Beyond Table~\ref{tab:steering}, evidence for the two paradigms comes from the identity and texture of the fallback. For Gemma, \emph{``A l\'ingua portuguesa \'e falada em''} continues in Spanish (\emph{``todos os continentes, enrapapeando la lengua espa\~nola...''}), and another prompt drifts into Catalan (\emph{``la una metr\`opolis ambduna de les fa uns centenars''})---a language absent from our five FLORES evaluation languages, indicating that the continuum is not a ranking over the measured set but a broader typological geometry that includes unevaluated Romance varieties. Gemma's curated multilingual training mix~\cite{lieberum2024gemma} plausibly encourages shared Romance structure. Llama, in contrast, falls back to English (\emph{``the longest the most long the is the more long the''} from \emph{``O rio mais longo do mundo \'e''}), consistent with Wendler et al.~\cite{wendler2024llamas} and with Meta's ${\sim}90\%$ English pretraining mix~\cite[Table~10]{touvron2023llama2} that would make English a natural base layer.
