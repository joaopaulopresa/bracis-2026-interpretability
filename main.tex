...
For Llama we restricted the manual-inspection scope to the top-20 features at the SAE-training layer (layer 10), whereas for Gemma the 120 features span all 6 analyzed layers; this asymmetry reflects the cost of token-level inspection of Llama-Scope's larger dictionary and the fact that Table~\ref{tab:features} shows feature counts already decrease with depth in Llama. After masking, the top features are genuinely linguistic: syntactic features (prepositions ``de''/``com'', articles ``a''/``os''/``um'') dominate, with morphological features (``s\~ao'', ``\^a'', suffixes ``-icos'') also present. This mirrors Gemma~2B's layer-10 features, where syntactic and morphological categories also predominate. The padding masking correction matters methodologically: differential analysis on padded sequences can produce spurious results when languages have different tokenization lengths, a pitfall we are not aware of having been explicitly documented in the SAE interpretability literature.
...
The Romance-language continuum observed in Gemma~2B organizes Romance languages (those descended from Latin: Portuguese, Spanish, French, Italian, Catalan) in a continuous representational space where typological proximity maps to representational proximity. When Portuguese features are suppressed, the prompt ``A l\'ingua portuguesa \'e falada em'' continues coherently in Spanish: \emph{``todos os continentes, enrapapeando la lengua espa\~nola, la misma lengua que es fa...''}. The model not only switches to Spanish but continues the topic of language distribution. In another example, Gemma's fallback drifts into Catalan (\emph{``la una metr\`opolis ambduna de les fa uns centenars''}), a language that does \emph{not} appear among our five FLORES evaluation languages (PT, EN, ES, FR, IT). That a suppressed-PT Gemma reaches for Catalan rather than for Spanish, Italian, or French in these cases indicates that the continuum is not merely a ranking over the five languages we measured but a broader typological geometry that includes Romance varieties absent from our evaluation set. Google's Gemma models use curated multilingual training data with deliberate language balancing~\cite{lieberum2024gemma}, which may encourage the model to learn shared Romance structure rather than treating each language independently.
